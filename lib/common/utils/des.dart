...
class DESEncrypt {
  ...
  static String generatekey(int offset) {
    String library = 'SFVvYVN0Uk9kZXZ3ZlZJMUtzTUNqR2lReDByY21ieVhnaE5rQkxGNVdUN3VEbnpQWTY0cDkyQTNKbHFFWjg=';
  ...
  static String encryptByDES(String message) {
    ...
    // );
    final encrypter = Encrypter(AES(keys, mode: AESMode.ecb, padding: 'PKCS7'));
  ...
  static String decryptByDES(String ciphertext, String type) {
    ...
    final keys = Key.fromUtf8(key);
    final encrypter = Encrypter(
      AES(keys, mode: AESMode.ecb, padding: 'PKCS7'),
    );
    final decrypted = encrypter.decrypt(Encrypted.fromBase64(ciphertext));

    String decVal = decrypted;
    if (type == 'phone') {
      decVal = decVal.replaceAllMapped(RegExp(r'(\d{3})\d{4}(\d{4})'), (Match m) => '${m.group(1)}****${m.group(2)}');
    }
    if (type == 'idCard') {
      decVal = decVal.replaceAllMapped(RegExp(r'(\d{3})\d{11}(\d{4})'), (Match m) => '${m.group(1)}***********${m.group(2)}');
    }
    return decVal;
  }
}
