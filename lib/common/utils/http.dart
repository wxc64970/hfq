...
class HttpUtil {
  ...
  HttpUtil._internal({String baseUrl = Constants.apiUrl}) {
  ...
  void onRequest(RequestOptions options) async {
    LogUtil.d("request.options -> ${options.uri}---${options.headers}");
  }

  /*
   * Response 统一验证
   */
  void onResponse(response) async {
    LogUtil.d('response.data -> $response');
    if (response.data["code"] != null) {
      if (response.data["code"] != 0) {
        if (response.data["msg"] != null) {
          SmartDialog.showToast(response.data["msg"].toString());
        }
      }
  ...
  void onError(ErrorEntity eInfo) {
    Loading.close();
    LogUtil.d('error.code -> ' + eInfo.code.toString() + ', error.message -> ' + eInfo.message);
    switch (eInfo.code) {
      case 401:
        UserStore.to.onLogout();
